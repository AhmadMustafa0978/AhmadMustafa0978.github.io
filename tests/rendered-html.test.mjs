...
test("server renders the LUTON landing page", async () => {
  const response = await render();
  assert.equal(response.status, 200);
  assert.match(response.headers.get("content-type") ?? "", /^text\/html\b/i);

  const html = await response.text();
  assert.match(html, /LUTON Lubricants/);
  assert.match(html, /زيوت تحمي/);
  assert.match(html, /مساعد اختيار الزيت/);
...
test("removes temporary starter preview assets", async () => {
  ...
  assert.match(layout, /LUTON Lubricants/);
